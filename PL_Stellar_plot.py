...
class PLS_plot(tk.Toplevel):
    ...
    def overlaying_plot(self):
        ...
        submit_Button = Button(self, text='submit', bg=HIERNtheme.myorange, fg=HIERNtheme.mywhite, font=HIERNtheme.standard_font, command=lambda:self.__PLS_double_plotting(self.row.get(), self.column.get()))
        submit_Button.grid(row=10, column=5)

        self.update_idletasks()
        self.deiconify()

    def PLS_plotting(chosen, completeName1, PL_corrected, GaussFits_PL, self):
        ...
        for k in range(0,96,2):
            ...
            x_abs[k] = PLS_fit.PL_corrected.iloc[:1200,k]
        ...
        for i in range(0,48):
            ...
            for i in range(0,8):
                ...
                if MyUtils.plate[i] in MyUtils.chosen:
                    axs[j,i-8*j].plot(x_abs[k], PLS_fit.PL_corrected.iloc[:1200,k+1], color=[red[i-8*j],green[j],blue[0],alpha[0]], linewidth=1.5)
                    #axs[j,i-8*j].plot(x_abs[k], PL_data.iloc[:,k+1], '--', color=[0.38,0.38,0.38,1], linewidth=1.5)
                    axs[j,i-8*j].plot(x_abs[k], PLS_fit.GaussFits_PL[i].best_fit, '--', color=[0.38,0.38,0.38,1], linewidth=1.5)
    ...
    def __PLS_double_plotting(self, row, column):
        ...
        if self.row.get() == 'A':
            for i in range(0,8):
                ...
                j = 0
                if MyUtils.plate[i] in MyUtils.chosen:
                    axs[0].plot(x_abs[k], PLS_fit.PL_corrected.iloc[:1200,k+1], color=[red[i-8*j],green[j],blue[0],alpha[0]], linewidth=1.5, label=MyUtils.plate[i])
        ...
        if self.column.get() == '1':
            for i in range (0,48,8):
                ...
                j = int(i/8)
                if MyUtils.plate[i] in MyUtils.chosen:
                    axs[1].plot(x_abs[k], PLS_fit.PL_corrected.iloc[:1200,k+1], color=[red[i-8*j],green[j],blue[0],alpha[0]], linewidth=1.5, label=MyUtils.plate[i])
        ...
        if self.column.get() == '2':
            for i in range (0,48,8):
                ...
                j = int(i/8)
                if MyUtils.plate[i] in MyUtils.chosen:
                    axs[1].plot(x_abs[k], PLS_fit.PL_corrected.iloc[:1200,k+1], color=[red[i+1-8*j],green[j],blue[0],alpha[0]], linewidth=1.5, label=str(MyUtils.plate[i+1]))
        ...
        if self.column.get() == '3':
            for i in range (0,48,8):
                ...
                j = int(i/8)
                if MyUtils.plate[i] in MyUtils.chosen:
                    axs[1].plot(x_abs[k], PLS_fit.PL_corrected.iloc[:1200,k+1], color=[red[i+2-8*j],green[j],blue[0],alpha[0]], linewidth=1.5, label=MyUtils.plate[i+2])
        ...
        if self.column.get() == '4':
            for i in range (0,48,8):
                ...
                j = int(i/8)
                if MyUtils.plate[i] in MyUtils.chosen:
                    axs[1].plot(x_abs[k], PLS_fit.PL_corrected.iloc[:1200,k+1], color=[red[i+3-8*j],green[j],blue[0],alpha[0]], linewidth=1.5, label=MyUtils.plate[i+3])
        ...
        if self.column.get() == '5':
            for i in range (0,48,8):
                ...
                j = int(i/8)
                if MyUtils.plate[i] in MyUtils.chosen:
                    axs[1].plot(x_abs[k], PLS_fit.PL_corrected.iloc[:1200,k+1], color=[red[i+4-8*j],green[j],blue[0],alpha[0]], linewidth=1.5, label=MyUtils.plate[i+4])
        ...
        if self.column.get() == '6':
            for i in range (0,48,8):
                ...
                j = int(i/8)
                if MyUtils.plate[i] in MyUtils.chosen:
                    axs[1].plot(x_abs[k], PLS_fit.PL_corrected.iloc[:1200,k+1], color=[red[i+5-8*j],green[j],blue[0],alpha[0]], linewidth=1.5, label=MyUtils.plate[i+5])
        ...
        if self.column.get() == '7':
            for i in range (0,48,8):
                ...
                j = int(i/8)
                if MyUtils.plate[i] in MyUtils.chosen:
                    axs[1].plot(x_abs[k], PLS_fit.PL_corrected.iloc[:1200,k+1], color=[red[i+6-8*j],green[j],blue[0],alpha[0]], linewidth=1.5, label=MyUtils.plate[i+6])
        ...
        if self.column.get() == '8':
            for i in range (0,48,8):
                ...
                j = int(i/8)
                if MyUtils.plate[i] in MyUtils.chosen:   
                    axs[1].plot(x_abs[k], PLS_fit.PL_corrected.iloc[:1200,k+1], color=[red[i+7-8*j],green[j],blue[0],alpha[0]], linewidth=1.5, label=MyUtils.plate[i+7])
    ...
    def PLS_plotting_norm(chosen, completeName2, PL_data_norm, GaussFits_PL_norm, PL_corrected, self):        
        ...
        for i in range(0,48):
            for i in range(0,8):
                ...
                axs[j,i-8*j].set_title(MyUtils.plate[i])
                if MyUtils.plate[i] in MyUtils.chosen:
                    axs[j,i-8*j].plot(x_abs[k], PLS_fit.PL_data_norm[i], color=[red[i-8*j],green[j],blue[0],alpha[0]], linewidth=1.5)
                    axs[j,i-8*j].plot(x_abs[k], PLS_fit.GaussFits_PL_norm[i], '--', color=[0.38,0.38,0.38,1], linewidth=1.5)
            for i in range(8,16):
                k = i*2
                j = 1
                axs[j,i-8*j].set_title(MyUtils.plate[i])
